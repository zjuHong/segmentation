...
def conv2d_basic(x, W, bias):       # strides 卷积时在图像每一维的步长，这是一个一维的向量
    conv = tf.nn.conv2d(x, W, strides=[1, 1, 1, 1], padding="SAME") # padding = "SAME"时，表示卷积核可以停留在图像边缘
    return tf.nn.bias_add(conv, bias)

def conv2d_atrous_2(x,W,bias):
    conv =tf.nn.atrous_conv2d(x,W,rate=2,padding="SAME")
    return tf.nn.bias_add(conv,bias)

def conv2d_atrous_4(x,W,bias):
    conv =tf.nn.atrous_conv2d(x,W,rate=4,padding="SAME")
    return tf.nn.bias_add(conv,bias)
...
def conv2d_transpose_strided8(x, W, b, output_shape=None, stride = 8):
    # print x.get_shape()
    # print W.get_shape()
    if output_shape is None:
        # x.shape = [b,h,w,c]
        # W.shape = [h,w, outc, inc]
        output_shape = x.get_shape().as_list()      # output_shape 如果没有出入，则默认将x扩大两倍
        output_shape[1] *= 8                        # x 中h*2
        output_shape[2] *= 8                        # w * 2
        output_shape[3] = W.get_shape().as_list()[2]    # output_shape中的通道个数 = W中输出通道个数outc
    # print output_shape
    conv = tf.nn.conv2d_transpose(x, W, output_shape, strides=[1, stride, stride, 1], padding="SAME")
    return tf.nn.bias_add(conv, b)
